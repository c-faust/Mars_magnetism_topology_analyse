...
- `--min-direction-valid-fraction 0.1`: discard timestamps where a used direction has too few positive finite energy bins. This prevents clustering from selecting spectra that are mostly filled/interpolated because pitch-angle coverage was sparse.
- `--normalization log|global_zscore|zscore|minmax|l2`: preprocessing method. Use `log` to keep parallel/anti-parallel flux differences in each paired sample.
